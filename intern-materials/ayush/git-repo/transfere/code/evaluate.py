...
def rollout(policy, pre, post, env, task_description, max_steps, seed, capture=False):
    ...
    for _ in range(max_steps):
        ...
        if info.get("is_success", False):
            if capture:
                frames.append(np.asarray(obs["pixels"]["image"])[::-1].copy())
            return (True, frames) if capture else True
        if terminated or truncated:
            break
    return (False, frames) if capture else False
...
def eval_indist(models, n_episodes, max_steps, results, out_path, suite, bench_name, n_tasks=N_TASKS):
    ...
    vid_root = Path(out_path).parent / "episode_videos"   # per-episode videos saved during testing
    for mname, (policy, pre, post) in models.items():
        ...
        for task_id in range(n_tasks):
            ...
            print(f"[{bench_name}] {mname} task{task_id}: {rate*100:.1f}%  "
                  f"({sum(succ)}/{n_episodes} PASS, videos -> episode_videos/{mname}/)", flush=True)
